...
\fancyfoot[C]{A Not-Too-Short Summary of \textit{Algorithms}}
\fancyfoot[R]{\thepage}
...
\begin{quote}
    \setlength{\leftskip}{0.25cm}
    $\textcolor{color2}{\Longrightarrow}$ The time complexity with memoization is \(O(n \cdot m)\), where \(n\) is the number of items and \(m\) is the capacity of the knapsack. This is because each subproblem \texttt{(level, capacity)} is solved at most once, leading to a polynomial time complexity.
\end{quote}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{knapsack\_naive}{$capacity, weights, values$} \Comment{\hlti{$O(n \cdot m)$}}
...
\begin{quote}
    \setlength{\leftskip}{0.25cm}
    $\textcolor{color2}{\Longrightarrow}$ For a step by step animation please refer to the following \href{https://docs.google.com/presentation/d/1MFxadjo6IOPm6Uh9uILxVhRI6qyjtzRjLMsHNQRhXPA/edit?usp=sharing}{link}.
\end{quote}

\begin{algorithm}
    \caption{0/1 Knapsack using Dynamic Programming}
    \begin{algorithmic}[1]
        \Function{knapsack}{$values, weight, capacity$} \Comment{Time Complexity: \hlti{$O(n \cdot m)$} | Space Complexity: \hlti{$O(n \cdot m)$}}
            \State $N \gets $ len($values$)
            \State $M \gets capacity$
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/knapsack-dp.png}
    \caption{Dynamic programming table for the 0/1 Knapsack problem with weights [1, 2, 3], values [6, 10, 12], and capacity of 5. Each cell $dp[i][m]$ represents the maximum profit for the first $i$ items with a capacity $m = 5$.}
    \label{fig:01knapsack}
\end{figure}
