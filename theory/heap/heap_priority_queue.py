...
class HeapPriorityQueue:
    def __init__(
        self, compare, temp_insert_value, get_key_func, set_key_func=None
    ):
        """
        Initialize minimum priority queue implemented with a heap.

        Arguments:
        compare -- comparison function: greater-than for a max-heap priority
        queue, less-than for a min-heap priority queue

        temp_insert_value -- temporary value given to objects upon insertion,
        then changed to the actual value of the object
    ...
    def update_key(self, x, k):
        ...
        k -- new key of x
        """
        # set the key to the new object which might have
        # higher or lower priority to the original one
        ...
        while i > 0 and \
                self.compare(
                    self.get_key(self.heap.get_array()[i]),
