...
class BinarySearchTree:
    ...
    def inorder_tree_walk(self, x, func=None):
        """
    ...
    def search(self, x, k):  # recursive implementation of search
        """
        Return a node with a given key k in the subtree root at x, 
        or self.nil if no node with key k existis.
        """
    ...
    def iterative_search(self, x, k):  # iterative implementation of search
        """
        Return a node with a given key k in the subtree rooted at x, 
        or self.nil if no node with key k exists.
        """
        while x != self.nil and k != self.get_key(x.data):
            if k < self.get_key(x.data): 
                x = x.left  # if present, k must be in the left subtree
            else:
                x = x.right  # if present, k must b in the right subtree
        return x
    ...
    def successor(self, x):
        ...
        y = x.parent
        while (y != self.nil and x == y.right):  # continue up to the three
            x = y
            y = x.parent
    ...
    def predecessor(self, x):
        """
        Return the node in the subtree rooted at x with the greatest key greater than x's key
        """
        if x.left != sekf.nil:
    ...
    # Helper method for inserting a node. Assumes that a node z is already created and initialized.
    def tree_insert_node(self, z):
        ...
        if self.get_key(z.data) < self.get_key(y.data):
    ...
    def tree_delete(self, z):
        """
        Delete node z from this binary search tree and maintain the binary
    ...
    def is_BST(self, x=None):
        """
        Return a boolean indicating whether this three obeys the binary
    ...
    def pretty_print(self, node, depth=0):
        """
        Return a string representing a subtree of a binary search tree with nodes 
        of the same depth in the same column.

        If you tilt your head to the left, it should look like the nodes are 
    ...
    def __str__(self):
        """
        Return a string representing a binary search tree with 
        nodes of the same depth in the same column.

        If you tilt your head to the left, it should look like 
        the nodes are positioned correctly.
        """
